...
const SettingItem = ({ icon, label, value, onClick, rightElement, danger }: SettingItemProps) => (
  <button
    onClick={onClick}
    style={{
      width: "100%",
...
export default function PerfilPage() {
  ...
  const [profilePhoto, setProfilePhoto] = useState<string | null>(() =>
    Storage.get<string | null>(`profilePhoto_${user?.id}`, null)
  );
  ...
  useEffect(() => {
    if (user?.id) {
      setProfilePhoto(Storage.get<string | null>(`profilePhoto_${user.id}`, null));
      setEditedName(user.nome);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- atualiza apenas quando ID muda, nao quando nome muda
  }, [user?.id]);
  ...
  const handleSaveName = () => {
    if (user && editedName.trim() && editedName !== user.nome) {
      setUser({ ...user, nome: editedName.trim() });
      ...
      <div>
        ...
        <div style={{ textAlign: "center", padding: "60px 40px" }}>
          ...
          <p style={{ color: "var(--text-muted)", marginBottom: "24px", fontSize: "14px" }}>
            Faca login para acessar o acervo
          </p>
        </div>
      </div>
    );
    ...
    <div>
      ...
      <div style={{ padding: "20px", display: "flex", flexDirection: "column", alignItems: "center" }}>
        ...
        <div style={{ position: "relative", marginBottom: "16px" }}>
          ...
          >
            {!profilePhoto && user.nome?.charAt(0).toUpperCase()}
          </div>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "14px", marginBottom: "8px", textAlign: "center" }}>
          {user.instrumento}
        </p>
      ...
      <div style={{ padding: "0 20px 20px" }}>
        ...
        <SettingItem
          ...
          value={`Versao ${APP_VERSION} - Dezembro 2025`}
          onClick={() => setShowAboutModal(true)}
        />

        {/* Sair */}
        <div style={{ marginTop: "32px" }}>
          <SettingItem icon={<Icons.Logout />} label="Sair da conta" onClick={handleLogout} danger />
        </div>
      </div>
