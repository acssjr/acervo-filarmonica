...
const EmptyState = ({ icon: Icon, title, subtitle, size = "default" }: EmptyStateProps) => {
  ...
  return (
    <div style={{
      ...
      {title && (
        <p style={{ fontFamily: "Outfit, sans-serif", fontSize: `${s.title}px`, marginBottom: subtitle ? "8px" : 0 }}>
          {title}
        </p>
      )}
      {subtitle && (
        <p style={{ fontFamily: "Outfit, sans-serif", fontSize: `${s.subtitle}px`, opacity: 0.7 }}>
          {subtitle}
        </p>
      )}
    </div>
