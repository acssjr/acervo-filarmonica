...
const FeaturedCard = memo(({ sheet, category, bgImage, isFav, onToggleFavorite, isDesktop, stopAnimation }: FeaturedCardProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Badge */}
      <div style={{ position: 'absolute', top: '12px', right: '12px', background: '#D4AF37', color: '#3D1518', fontSize: '9px', fontFamily: "var(--font-sans)", fontWeight: '700', padding: '4px 8px', borderRadius: '8px', textTransform: 'uppercase', letterSpacing: '0.5px', zIndex: 2 }}>Em estudo</div>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <h4 style={{ fontSize: '16px', fontWeight: '700', marginBottom: '4px', color: '#FFFFFF', fontFamily: "var(--font-sans)", whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{sheet.title}</h4>
        <p style={{ fontSize: '13px', fontFamily: "var(--font-sans)", color: 'rgba(255, 255, 255, 0.7)', marginBottom: '14px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{sheet.composer}</p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ padding: '5px 10px', background: 'rgba(212, 175, 55, 0.2)', borderRadius: '8px', border: '1px solid rgba(212, 175, 55, 0.3)', flexShrink: 1, minWidth: 0, maxWidth: '70%', overflow: 'hidden' }}>
            <span style={{ fontSize: '11px', fontFamily: "var(--font-sans)", color: '#D4AF37', fontWeight: '600', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'block' }}>{category?.name}</span>
          </div>
