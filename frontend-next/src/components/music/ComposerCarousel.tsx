...
const ComposerCarousel = ({ composers = [] }: ComposerCarouselProps) => {
  ...
  return (
    <div style={{ padding: '24px 0 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 20px', marginBottom: '16px' }}>
        <h2 style={{ fontFamily: "var(--font-sans)", fontSize: '18px', fontWeight: '700' }}>Compositores</h2>
        <button
          style={{ background: 'none', border: 'none', color: 'var(--primary)', fontSize: '14px', fontWeight: '500', cursor: 'pointer', fontFamily: "var(--font-sans)" }}
          ...
          {(hasInteracted || topComposers.length <= 1 ? topComposers : duplicatedComposers).map((composer, index) => {
            ...
            return (
              <button
                ...
                onClick={() => router.push('/compositores')}
                style={{
                  flex: '0 0 220px', width: '220px', height: '140px', borderRadius: '14px',
              ...
              >
                {hasPhoto && (
                  <div style={{ position: 'absolute', inset: '-10%', backgroundImage: `url(${hasPhoto})`, backgroundSize: 'cover', backgroundPosition: 'center 30%', filter: 'brightness(0.85)' }} />
                )}

                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.95) 0%, rgba(0,0,0,0.5) 50%, transparent 80%)' }} />

                <div style={{ position: 'absolute', bottom: '10px', left: '10px', right: '10px', padding: '10px 12px', borderRadius: '10px', background: 'rgba(255, 255, 255, 0.1)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', border: '1px solid rgba(255, 255, 255, 0.15)', textAlign: 'left' }}>
                  <p style={{ fontFamily: "var(--font-sans)", fontSize: '14px', fontWeight: '700', color: '#FFFFFF', marginBottom: '2px', textShadow: '0 1px 2px rgba(0,0,0,0.3)' }}>
                    {composer.name}
                  </p>
                  <p style={{ fontFamily: "var(--font-sans)", fontSize: '11px', color: 'rgba(255, 255, 255, 0.8)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                {isFirst && (
                  <div style={{ position: 'absolute', top: '10px', right: '10px', padding: '3px 8px', borderRadius: '20px', background: 'linear-gradient(145deg, #D4AF37 0%, #B8860B 100%)', fontSize: '10px', fontWeight: '600', color: '#1A1A1A', fontFamily: "var(--font-sans)", boxShadow: '0 2px 8px rgba(212, 175, 55, 0.4)' }}>
                    Destaque
                  </div>
                )}
              </button>
