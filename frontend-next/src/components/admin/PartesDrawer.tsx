...
const PartesDrawer = ({ isOpen, onClose, partitura, categorias, onUpdate }: PartesDrawerProps) => {
  ...
  // Substituir parte
  const handleReplacePart = async (parteId: string | number, file: File) => {
    if (!file || !partitura?.id) return;
  ...
  // Deletar parte
  const handleDeletePart = async (parteId: string | number) => {
    if (!confirm('Remover esta parte da partitura?') || !partitura?.id) return;
  ...
  const handleAddPart = async (file: File) => {
    if (!file || !newPartInstrumento.trim() || !partitura?.id) {
  ...
  // Abrir PDF em nova aba (via fetch+blob para enviar auth header)
  const handleViewPart = async (parte: Parte) => {
    if (!partitura?.id) return;
    try {
      const blob = await API.downloadBlob(`/api/download/parte/${parte.id}`);
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '20px 24px' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>
              ...
              <p style={{ fontSize: '13px', marginTop: '8px' }}>Adicione uma parte usando o botao abaixo</p>
            </div>
          ) : (
