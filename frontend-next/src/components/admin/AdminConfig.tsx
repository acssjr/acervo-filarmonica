...
const AdminConfig = () => {
  ...
  const handleToggleRecesso = async () => {
    ...
    try {
      ...
      showToast(
        novoValor ? "Modo recesso ativado" : "Modo recesso desativado"
      );
    } catch {
      setModoRecesso(!novoValor);
  ...
  const handlePhotoUpload = async (
    ...
    try {
      ...
      if (result.url && user) {
        setUser({ ...user, foto_url: result.url });
  ...
  const handleRemovePhoto = async () => {
    try {
      await API.updatePerfil({ foto_url: null });
      if (user) {
        setUser({ ...user, foto_url: undefined });
      }
