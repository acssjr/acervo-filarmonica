...
const AdminPartituras = () => {
  ...
  return (
    <div className="page-transition" style={{ padding: '32px', maxWidth: '1000px', margin: '0 auto', fontFamily: 'Outfit, sans-serif' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <button
            ...
            style={{
              ...
              cursor: (tutorialPending || showTutorial) ? 'not-allowed' : 'pointer', fontFamily: 'Outfit, sans-serif',
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        <div style={{ position: 'relative', minWidth: '200px' }}>
          <button type="button" onClick={() => setShowCatDropdown(!showCatDropdown)} style={{
            ...
            fontSize: '14px', fontFamily: 'Outfit, sans-serif', textAlign: 'left', cursor: 'pointer',
          ...
          {showCatDropdown && (
            <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, background: 'var(--bg-secondary)', border: '1.5px solid var(--border)', borderRadius: '12px', boxShadow: '0 8px 24px rgba(0,0,0,0.15)', zIndex: 100, maxHeight: '280px', overflowY: 'auto' }}>
              <button onClick={() => { setFilterCategoria(''); setShowCatDropdown(false); }} style={{ width: '100%', padding: '12px 16px', border: 'none', background: !filterCategoria ? 'rgba(212, 175, 55, 0.1)' : 'transparent', color: !filterCategoria ? '#D4AF37' : 'var(--text-primary)', fontSize: '14px', fontFamily: 'Outfit, sans-serif', textAlign: 'left', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' }}>
                Todas categorias
              </button>
              {categorias.map(cat => (
                <button key={cat.id} onClick={() => { setFilterCategoria(cat.id); setShowCatDropdown(false); }} style={{ width: '100%', padding: '12px 16px', border: 'none', background: filterCategoria === cat.id ? 'rgba(212, 175, 55, 0.1)' : 'transparent', color: filterCategoria === cat.id ? '#D4AF37' : 'var(--text-primary)', fontSize: '14px', fontFamily: 'Outfit, sans-serif', textAlign: 'left', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ width: '24px', height: '24px', borderRadius: '6px', background: 'linear-gradient(145deg, #3a3a4a 0%, #2a2a38 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(212, 175, 55, 0.2)' }}>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)', fontFamily: 'Outfit, sans-serif' }}>Carregando...</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-secondary)', fontFamily: 'Outfit, sans-serif' }}>
          ...
          {sortedLetters.map((letter) => (
            <div key={letter}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px', paddingBottom: '8px', borderBottom: '2px solid var(--border)' }}>
                <div style={{ width: '36px', height: '36px', borderRadius: '8px', background: 'linear-gradient(135deg, #D4AF37 0%, #B8860B 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: '700', color: '#fff', fontFamily: 'Outfit, sans-serif', boxShadow: '0 2px 6px rgba(212, 175, 55, 0.3)' }}>
                  {letter}
                </div>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: 'var(--bg-card)', borderRadius: '24px', padding: '0', zIndex: 1001, width: '440px', maxWidth: '90vw', maxHeight: '90vh', overflow: 'hidden', boxShadow: '0 25px 60px rgba(0, 0, 0, 0.4)', animation: 'scaleIn 0.25s cubic-bezier(0.16, 1, 0.3, 1)' }}>
            <div style={{ padding: '24px 24px 20px', borderBottom: '1px solid var(--border)', background: 'linear-gradient(180deg, var(--bg-secondary) 0%, var(--bg-card) 100%)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: 'var(--text-primary)', fontFamily: 'Outfit, sans-serif' }}>Editar Partitura</h2>
                  <p style={{ margin: '4px 0 0', fontSize: '13px', color: 'var(--text-muted)', fontFamily: 'Outfit, sans-serif', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{editingPartitura.titulo}</p>
                </div>
            ...
            <div style={{ padding: '24px' }}>
              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: 'var(--text-secondary)', marginBottom: '8px', fontFamily: 'Outfit, sans-serif' }}>Titulo da Partitura</label>
                <input type="text" value={editForm.titulo} onChange={(e) => setEditForm(prev => ({ ...prev, titulo: e.target.value }))} placeholder="Nome da partitura" style={{ width: '100%', padding: '14px 16px', borderRadius: '12px', border: '1.5px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '15px', fontFamily: 'Outfit, sans-serif', outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s', boxSizing: 'border-box' }} onFocus={(e) => { e.target.style.borderColor = '#D4AF37'; e.target.style.boxShadow = '0 0 0 3px rgba(212, 175, 55, 0.15)'; }} onBlur={(e) => { e.target.style.borderColor = 'var(--border)'; e.target.style.boxShadow = 'none'; }} />
              </div>
              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: 'var(--text-secondary)', marginBottom: '8px', fontFamily: 'Outfit, sans-serif' }}>Compositor</label>
              ...
              <div style={{ marginBottom: '28px' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: 'var(--text-secondary)', marginBottom: '8px', fontFamily: 'Outfit, sans-serif' }}>Categoria</label>
                <div style={{ position: 'relative' }}>
                  <select value={editForm.categoria_id} onChange={(e) => setEditForm(prev => ({ ...prev, categoria_id: e.target.value }))} style={{ width: '100%', padding: '14px 16px', paddingRight: '44px', borderRadius: '12px', border: '1.5px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '15px', fontFamily: 'Outfit, sans-serif', outline: 'none', cursor: 'pointer', appearance: 'none', transition: 'border-color 0.2s, box-shadow 0.2s' }} onFocus={(e) => { e.target.style.borderColor = '#D4AF37'; e.target.style.boxShadow = '0 0 0 3px rgba(212, 175, 55, 0.15)'; }} onBlur={(e) => { e.target.style.borderColor = 'var(--border)'; e.target.style.boxShadow = 'none'; }}>
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                <button onClick={closeEditModal} className="btn-ghost-hover" style={{ flex: 1, padding: '14px 20px', borderRadius: '12px', border: '1.5px solid var(--border)', background: 'transparent', color: 'var(--text-secondary)', fontSize: '15px', fontWeight: '600', fontFamily: 'Outfit, sans-serif', cursor: 'pointer', transition: 'all 0.2s' }}>Cancelar</button>
                <button onClick={saveEditModal} disabled={savingEdit || !editForm.titulo.trim()} className="btn-primary-hover" style={{ flex: 1, padding: '14px 20px', borderRadius: '12px', border: 'none', background: savingEdit || !editForm.titulo.trim() ? 'linear-gradient(135deg, rgba(212, 175, 55, 0.5) 0%, rgba(184, 134, 11, 0.5) 100%)' : 'linear-gradient(135deg, #D4AF37 0%, #B8860B 100%)', color: '#fff', fontSize: '15px', fontWeight: '600', fontFamily: 'Outfit, sans-serif', cursor: savingEdit || !editForm.titulo.trim() ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'all 0.2s', boxShadow: savingEdit || !editForm.titulo.trim() ? 'none' : '0 4px 12px rgba(212, 175, 55, 0.3)' }}>
